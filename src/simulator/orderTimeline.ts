...
export function advanceOrder(order: ActiveOrder, input: { elapsedMinutes: number; efficiency: number; droneBaseMinutes?: number }) {
  ...
  const natural = elapsedMinutes * (order.base.efficiencyAffected ? efficiency : 1)
  ...
  return {
    ...
    unusedMinutes: efficiency > 0 ? unusedNaturalWork / (order.base.efficiencyAffected ? efficiency : 1) : 0,
