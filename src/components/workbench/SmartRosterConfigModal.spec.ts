...
describe('SmartRosterConfigModal.vue', () => {
  ...
  it('renders modal content when open is true', () => {
    ...
    expect(wrapper.find('[data-test="enable-surrogate"]').exists()).toBe(true)
    expect(wrapper.find('[data-test="surrogate-samples-input"]').exists()).toBe(true)
    expect(wrapper.find('[data-test="surrogate-topk-input"]').exists()).toBe(true)
  ...
  it('initializes with default config and props', () => {
    ...
    const surrogateCheckbox = wrapper.find<HTMLInputElement>('[data-test="enable-surrogate"]')
    expect(surrogateCheckbox.element.checked).toBe(true)

    const surrogateSamplesInput = wrapper.find<HTMLInputElement>('[data-test="surrogate-samples-input"]')
    expect(Number(surrogateSamplesInput.element.value)).toBe(DEFAULT_CONFIG.surrogateSampleCount)
  ...
  it('emits confirm with modified parameters and persists to localStorage', async () => {
    ...
    await wrapper.find<HTMLInputElement>('[data-test="surrogate-samples-input"]').setValue(50000)
    await wrapper.find<HTMLInputElement>('[data-test="surrogate-topk-input"]').setValue(32)
    ...
    expect(stored.surrogateSampleCount).toBe(50000)
    expect(stored.surrogateTopK).toBe(32)
  })
