...
function executeAutoGenerate(inventoryEntries: OwnedOperatorInput[], config?: SmartRosterConfig): void {
  ...
  const runOptions = {
    ...
    enableSurrogate: config?.enableSurrogate ?? !isVitest,
    surrogateSampleCount: config?.surrogateSampleCount ?? (isVitest ? 5000 : 25000),
    surrogateTopK: config?.surrogateTopK ?? (isVitest ? 8 : 16),
  }
