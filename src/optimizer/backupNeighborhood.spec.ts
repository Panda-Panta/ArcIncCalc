...
function fixture(){
 ...
 for(const [key,name,replacements] of [['room_1_1','砾',['芬','香草']],['room_1_2','调香师',['红豆','Castle-3']]] as const){
...
describe('fair protected ordinary backup neighborhood',()=>{
 ...
 it('swaps adjacent priorities without changing the candidate set',()=>{
  ...
  expect(r[0]!.workspace.mainPlan.facilities.room_1_1.slots[0]!.replacements).toEqual(['香草','芬'])
 ...
 it('exchanges distinct slots at the same facility type and preserves literal names',()=>{
  ...
  expect(r.workspace.mainPlan.facilities.room_1_1.slots[0]!.replacements).toEqual(['红豆','香草'])
  expect(r.workspace.mainPlan.facilities.room_1_2.slots[0]!.replacements).toEqual(['芬','Castle-3'])
 })
 it('rejects exchanges across facility types or without matching owned maximum skills',()=>{
  ...
  expect(generateBackupNeighbors(w,allOwned,21).some(n=>n.move.kind==='exchange')).toBe(false)
  w.mainPlan.facilities.room_1_2.slots[0]!.replacements=['红豆']
  expect(generateBackupNeighbors(w,owned(['砾','调香师','芬','香草']),21).some(n=>n.move.kind==='exchange')).toBe(false)
 })
 it.each(['run','policy','fia','metadata','workaholic'])('freezes protected slots for every move: %s',protection=>{
  ...
  if(protection==='run')slot.replacements=['芬','但书']
  if(protection==='policy')w.mainPlan.conf.rest_in_full=['芬']
 ...
 it('normalizes name and ID aliases to avoid duplicate candidates and no-op swaps',()=>{
  const w=fixture();w.mainPlan.facilities.room_1_1.slots[0]!.replacements=['芬',id('红豆')];w.mainPlan.facilities.room_1_2.slots[0]!.replacements=[id('芬'),'Castle-3']
 ...
 it('does not move unowned or lower-stage existing candidates even during reordering',()=>{
  const w=fixture();w.mainPlan.facilities.room_1_1.slots[0]!.replacements=['芬','调香师']
  w.mainPlan.facilities.room_1_2.slots[0]!.occupant={kind:'operator',operatorId:id('断罪者')}
  const entries=[...owned(['砾','断罪者','芬','香草','红豆','Castle-3']),{operator:'调香师',elitePhase:0,level:1}]
 ...
 it('protects named staff inside nested custom policies and rejects invalid alias rosters',()=>{
  const w=fixture();w.mainPlan.conf.custom={target:{operator:id('芬')}}
  ...
  w.mainPlan.facilities.room_1_1.slots[0]!.replacements=['芬',id('芬')]
