...
vi.mock('../workbench/scheduleSimulationBridge',()=>({runScheduleSimulationBridge:vi.fn(()=>({report:{success:true,observedHours:72,operators:[],diagnostics:[],production:{sample:{completed:{exp:1000,gold:100,orderLmd:5000}}}}}))}))
...
describe('complete branch admission before any simulation',()=>{
 it('ignores old trial and top-K truncation and simulates all ten distinct branches in ideal mode',()=>{
 ...
 it('does not rank a partial simulation even when its incomplete production is very high',()=>{
  vi.mocked(runScheduleSimulationBridge).mockClear()
 ...
 it('does not start simulation if the required set cannot be constructed',()=>{
  vi.mocked(runScheduleSimulationBridge).mockClear()
