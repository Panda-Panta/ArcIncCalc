...
分支名：`mode/standard`。该版本面向常规持续生产与排班模拟，贸易站不提供“跑单”订单类型；现有通用方案会迁移到本分支的独立本地存储，历史跑单设置会转为普通订单。

对应的跑单版本位于 `mode/shift-run`，两个分支共享干员数据、组合、心情和收益算法，但具有独立的默认配置与浏览器存储。
